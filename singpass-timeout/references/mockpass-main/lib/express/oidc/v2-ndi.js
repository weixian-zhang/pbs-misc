      ...
      async (req, res) => {
        const {
          ...
          clientAssertion = {
            iss: 'mockpass',
            sub: 'mockpass',
            aud: 'http://localhost:5156/singpass/v2/token',
        ...
        //sleep on purpose to reproduce timeout
        var rand = Math.floor(Math.random() * 11);
        if (rand <= 3) { // 30% chance to timeout
        ...
        const rpJwksEndpoint = "http://127.0.0.1:8080/eservices/broker/mp/rp/jwks";
          // idp === 'singPass'
        ...
        // Step 2: Get ID token
        const aud = sub; //clientAssertionClaims['sub']
